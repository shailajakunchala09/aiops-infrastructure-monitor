...
# Consecutive breaches required before opening an incident
# (reduces noise/flapping)
...
def evaluate_metric_thresholds(
    ...
    """Compare a freshly ingested metric sample against server thresholds."""
    ...
    checks = [
        (
            AlertMetricType.CPU,
            metric.cpu_percent,
            server.cpu_threshold,
        ),
        (
            AlertMetricType.MEMORY,
            metric.memory_percent,
            server.memory_threshold,
        ),
        (
            AlertMetricType.DISK,
            metric.disk_percent,
            server.disk_threshold,
        ),
    ]
    ...
    for metric_type, observed, threshold in checks:
        if observed >= threshold:
            alert = Alert(
                ...
                message=(
                    f"{metric_type.value} usage on {server.hostname} "
                    f"reached {observed:.1f}% "
    ...
    if triggered:
        db.flush()
        _maybe_open_incident_for_alerts(
            db,
            server,
            triggered,
        )
...
def _maybe_open_incident_for_alerts(
    ...
    """
    Auto-creates an incident if there is not already an open incident
    for this server/metric combination.

    This avoids duplicate incidents for the same ongoing problem.
    """
    ...
    for alert in alerts:
        ...
        existing_open = (
            ...
            .filter(
                ...
                Incident.source
                == IncidentSource.AUTO_METRIC_THRESHOLD,
                Incident.title.like(
                    f"%{alert.metric_type.value}%"
                ),
            )
            .first()
        )
        ...
        incident = Incident(
            server_id=server.id,
            title=(
                f"High {alert.metric_type.value} utilization "
        ...
        logger.error(
            ...
            f"{incident.title} [{severity.value}]"
        )
...
def _severity_for_metric(
    ...
    """Calculate incident severity from threshold overage."""
...
def evaluate_log_for_incident(
    db: Session,
    server: Server,
    log_entry: LogEntry,
) -> Incident | None:
    """
    Auto-creates an incident when repeated CRITICAL errors
    are logged within a short time window.
    """
    ...
    incident = Incident(
        server_id=server.id,
        title=(
            f"Repeated critical errors from "
            f"{log_entry.source_application} "
            f"on {server.hostname}"
        ),
        description=(
            f"{recent_critical_count} CRITICAL log entries "
            f"recorded in the last 5 minutes. "
    ...
    logger.error(
        f"INCIDENT auto-created from logs: "
        f"{incident.title}"
    )
